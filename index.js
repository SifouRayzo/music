...
bot.on('ready', () => {
    ...
    bot.user.setActivity(`Events | DzOtaku Sys`);
}); // bot activity
...
bot.on("message", async msg => {
  ...
  if (command === "help" || command == "cmd") {
    const helpembed = new Discord.MessageEmbed()
      .setColor("#000000")
      .setAuthor(bot.user.tag, bot.user.displayAvatarURL()).setDescription(`
__**Commands List**__
> \`play\` > **\`play [title/url]\`**
> \`search\` > **\`search [title]\`**
> \`skip\`, \`stop\`,  \`pause\`, \`resume\`
> \`nowplaying\`, \`queue\`, \`volume\``)
    .setFooter("EventMusicBot | DzOtakuSys", "");
...
Please provide a value to select one of the search results ranging from 1-10.
    ...
    if (isNaN(args[1]) || args[1] > 150)
      return msg.channel.send(
        "Volume only can be set in range **1** - **150**."
      );
    serverQueue.volume = args[1];
    serverQueue.connection.dispatcher.setVolume(args[1] / 150);
